...
// ─── Shared mock data (dùng khi DB trống hoặc nguồn là Spoonacular) ───────────
...
export class SpoonacularIngredientRepository implements IIngredientRepository {
  async searchIngredients(keyword: string): Promise<IIngredient[]> {
    ...
    return response.data.map((item: any) => ({
      ...
      name: item.name,
      image: item.image
        ? `${SPOONACULAR_INGREDIENT_CDN}/${item.image}`
        : '',
    }));
  }
...
export class SupabaseIngredientRepository implements IIngredientRepository {
  ...
  async getPopularIngredients(): Promise<IIngredient[]> {
    try {
      ...
      // Fallback về mock nếu DB trống
      if (!data || data.length === 0) {
        console.log('ℹ️ ingredients table trống, dùng mock data thay thế.');
        return MOCK_POPULAR_INGREDIENTS;
      }
      ...
      console.error('🔴 Supabase getPopularIngredients error, fallback to mock:', error.message);
      return MOCK_POPULAR_INGREDIENTS;
    }
  }
}
