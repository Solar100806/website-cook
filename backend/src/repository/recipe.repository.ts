...
export interface IRecipeRepository {
  ...
  searchRecipesByIngredients(ingredients: string[]): Promise<RecipeByIngredientItem[]>;
...
/**
 * Throws if Supabase returns an error object — keeps each query site clean.
 */
...
/**
 * Builds a normalized ExtendedIngredient from a recipe_ingredients join row.
 */
...
export class SpoonacularRecipeRepository implements IRecipeRepository {
  async searchRecipes(query: string, cuisine: string): Promise<RecipeSearchResult> {
  ...
  async searchRecipesByIngredients(ingredients: string[]): Promise<RecipeByIngredientItem[]> {
    const response = await axios.get(
